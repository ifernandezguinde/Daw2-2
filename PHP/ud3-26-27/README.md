# Framework DAW2 (versión web)
Versión para final de curso que contiene cosas que se han ido añadiendo durante el progreso de las prácticas.
## Contenidos esta versión
* Las rutas en el menú lateral se comprueban contra `$_SERVER['REQUEST_URI']`.
* Se ha añadido soporte para mensajes flash `BaseController->addFlashMessage(Mensaje $message)`. Necesario habilitar las sesiones para que funcionen.
* Renombrado de tabla usuarios a trabajadores.
* Proveedores sample añadido.
* Contiene carpeta data/ con csv
* Tabla producto tiene id_producto AI como PK y codigo como UNIQUE NOT NULL (clave candidata)
